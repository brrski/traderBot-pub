...
class StockAnalyzer:
    def __init__(self):
        self.symbols = ['QQQ', 'SPY', 'TSM', 'AMD', 'NVDA', 'META', 'AAPL', 'TSLA', 'PLTR']
        self.price_data = {}
    ...
    def update_data(self):
        ...
        for symbol in self.symbols:
            ...
            # Get current price using fast_info
                try:
                    current_price = ticker.fast_info['lastPrice']
                    print(f"Current price for {symbol}: {current_price}")
                except:
                    print(f"Could not get current price for {symbol}, falling back to history")
                    current_price = None
            ...
            # Get historical data
                ...
                if df.empty:
                    print(f"Retrying {symbol} with shorter period...")
                    df = ticker.history(period="1mo")
            
                if not df.empty:
                    self.price_data[symbol] = df
                
                # If we couldn't get current price from fast_info, use latest close
                ...
                # Fetch options data
                    ...
                    try:
                        df = yf.download(symbol.strip(), start=start_date, end=end_date)
                        if not df.empty:
                            self.price_data[symbol] = df
                            if current_price is None:
                                current_price = float(df['Close'][-1])
                            print(f"Successfully fetched data using download method for {symbol}")
                    except Exception as e:
                        print(f"Download method also failed for {symbol}: {str(e)}")
